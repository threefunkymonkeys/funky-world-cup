class MatchPrediction < Sequel::Model
  ...
  def update_score(result)
    if host_score == result.host_score && rival_score == result.rival_score
      ...
      UserNotification.create(user_id: user_id, match_prediction_id: id, message: 'exact_match_penalty')
      ...
      UserNotification.create(user_id: user_id, match_prediction_id: id, message: 'partial_match_penalty')
    end
  ...
  def total_prediction_score
    prediction_score + (match_penalties_prediction ? match_penalties_prediction.prediction_score : 0)
  end
end
